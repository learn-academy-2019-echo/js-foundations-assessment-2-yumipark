...
console.log(numChecker(15))
console.log(numChecker(0))
console.log(numChecker(385))
console.log(numChecker(-7))
...
var helloMe = {
    ...
    getData: function() { return "My name is " + this.firstName + " " + this.lastName + ". I am " + this.ethnicity + " and I am from " + this.hometown + "." }
}
...
function lengthReturner(arr){
    let newArr = arr.map(x => x.length)
    return newArr
}

console.log(lengthReturner(randomNouns))
...
function spicyAlphabetSoup(str, str1, str2, str3){
    let newStr = str.concat(str1 + str2 + str3)
    let newArr = newStr.split("").sort().join("")
    return newArr 
}

console.log(spicyAlphabetSoup(newStr, testString1, testString2, testString3))
...
function animalsAmounts(arr1, arr2){
    let newArr = arr2.map((a, i) => `${arr1[i]} ${a}`).join(" ")
    return newArr
}
